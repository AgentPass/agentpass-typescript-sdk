...
/**
 * Complete MCP Server with Built-in API
 * 
 * This script runs both:
 * 1. A real Express API server on localhost
 * 2. An MCP server that converts the API endpoints to tools for Claude
 * 
 * Usage: node complete-mcp-server.mjs
 */

import { AgentPass } from './dist/core/AgentPass.js';
import express from 'express';

async function startCompleteMCPServer() {
  console.error('🚀 Starting Complete MCP Server with Built-in API...');
  ...
  try {
    ...
    // Generate MCP server with the REAL API URL
    ...
    console.error(`         "args": ["${process.cwd()}/complete-mcp-server.mjs"]`);
    console.error('       }');
    console.error('     }');
    console.error('   }');
    console.error('');
