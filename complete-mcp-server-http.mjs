 ...
 * 1. A real Express API server on localhost
 * 2. An MCP server that converts the API endpoints to tools for Claude (HTTP transport)
 * 
 * This version uses HTTP transport suitable for Claude Desktop web interface
 * or other HTTP-based MCP clients.
 * 
 * Usage: node complete-mcp-server-http.mjs
 */

import { AgentPass } from './dist/core/AgentPass.js';
import express from 'express';

async function startCompleteMCPServerHTTP() {
  console.error('🚀 Starting Complete MCP Server with Built-in API (HTTP Transport)...');

  try {
    // =============================================================================
    // STEP 1: START REAL EXPRESS API SERVER
    // =============================================================================
    console.error('📦 Starting Express API server...');
    
    const app = express();
    app.use(express.json());

    // Real API endpoints with actual data
    ...
    console.error(`✅ Express API server running on http://localhost:${apiPort}`);

    // =============================================================================
    // STEP 2: CREATE MCP SERVER FROM API ENDPOINTS
    // =============================================================================
    console.error('🔧 Creating MCP server from API endpoints...');
    ...
    const mcpServer = await agentpass.generateMCPServer({
      ...
      transport: 'http', // Using HTTP transport instead of stdio
      port: 0, // Use random available port
      host: 'localhost',
      cors: true, // Enable CORS for web clients
      baseUrl: `http://localhost:${apiPort}`, // Use the real API server port
      toolNaming: (endpoint) => {
    ...
    console.error(`   API Server: http://localhost:${apiPort}`);
    ...
    console.error('');
    console.error('🔧 Test MCP Server with curl:');
    console.error('');
    console.error('   # List available tools');
    console.error(`   curl -X POST ${mcpAddress}/mcp \\`);
    console.error('     -H "Content-Type: application/json" \\');
    console.error('     -d \'{"jsonrpc": "2.0", "id": 1, "method": "tools/list", "params": {}}\'');
    console.error('');
    console.error('   # Call a tool (example: get users)');
    console.error(`   curl -X POST ${mcpAddress}/mcp \\`);
    console.error('     -H "Content-Type: application/json" \\');
    console.error('     -d \'{"jsonrpc": "2.0", "id": 2, "method": "tools/call", "params": {"name": "get_users", "arguments": {"limit": 5}}}\'');
    console.error('');
    console.error('   # Call a tool with parameters (example: get user by ID)');
    console.error(`   curl -X POST ${mcpAddress}/mcp \\`);
    console.error('     -H "Content-Type: application/json" \\');
    console.error('     -d \'{"jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": {"name": "get_users_by_id", "arguments": {"id": 1}}}\'');
    console.error('');
    console.error('🎯 For Claude Desktop HTTP configuration:');
    console.error('   Note: Claude Desktop typically uses stdio transport, but you can');
    console.error('   configure web-based MCP clients to use this HTTP endpoint.');
    console.error('');
    console.error('📱 Integration Examples:');
    console.error('   - Web-based AI assistants');
    console.error('   - Custom applications using MCP over HTTP');
    console.error('   - Testing and development tools');
    console.error('');
    console.error('💡 This server includes both the API and MCP server in one process!');
    console.error(`   API Server: http://localhost:${apiPort}`);
    console.error(`   MCP Server: ${mcpAddress}`);
    console.error('');
    ...
    const cleanup = async () => {
      ...
      try {
        ...
        console.error('❌ Error during MCP server shutdown:', error.message);
      }
      process.exit(0);
    };
    ...
    console.error('❌ Failed to start complete MCP server:', error.message);
    console.error('Stack trace:', error.stack);
    process.exit(1);
  }
}

// Start the complete MCP server with HTTP transport
startCompleteMCPServerHTTP();