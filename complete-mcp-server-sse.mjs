 ...
 * 2. An MCP server that converts the API endpoints to tools for Claude (SSE transport)
 * 
 * This version uses SSE (Server-Sent Events) transport suitable for Claude Desktop
 * with SSE support or other SSE-based MCP clients.
 * 
 * Usage: node complete-mcp-server-sse.mjs
 */

import { AgentPass } from './dist/core/AgentPass.js';
import express from 'express';

async function startCompleteMCPServerSSE() {
  console.error('🚀 Starting Complete MCP Server with Built-in API (SSE Transport)...');
  ...
  try {
    ...
    const mcpServer = await agentpass.generateMCPServer({
      ...
      version: '1.0.0',
      description: 'MCP Server for Company Management - SSE Transport for Claude Desktop',
    ...
    console.error('');
    console.error('🔧 Connect with SSE client (MCP Spec):');
    console.error(`   GET ${mcpAddress}/sse         # Establish SSE stream`);
    console.error(`   POST ${mcpAddress}/sse/messages  # Send MCP messages`);
    console.error('');
    console.error('🎯 For Claude Desktop SSE configuration:');
    console.error('   Add this server address to your MCP client that supports SSE transport');
    console.error('');
    console.error('📱 Integration Examples:');
    console.error('   - Claude Desktop with SSE support');
    console.error('   - Custom MCP clients using SSE transport');
    console.error('   - Web-based AI assistants with SSE');
    console.error('');
    console.error('💡 This server includes both the API and MCP server in one process!');
    console.error(`   API Server: http://localhost:${apiPort}`);
    console.error(`   MCP SSE Server: ${mcpAddress}`);
    console.error('');
...
// Start the complete MCP server with SSE transport
startCompleteMCPServerSSE();